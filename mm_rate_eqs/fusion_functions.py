...
def get_sigma_fusion_approx(E, reaction='D_T_to_n_alpha'):
    """
    ...
    if reaction == 'D_D_to_p_T_n_He3':
        ...
        return sum([get_sigma_fusion_approx(E, reaction=reaction) for reaction in reactions])
...
def get_fusion_cross_section_data_file_path(reaction='D_T_to_n_alpha'):
    file = os.path.dirname(__file__) + '/fusion_cross_sections_data/' + reaction + '.csv'
    return file


def get_sigma_fusion_sampled(E_COM, reaction='D_T_to_n_alpha'):
    """
    Sampling of plots of sigma (cross sections) instead of using analytic fits.
    From Atzeni2004 Book, except the separate D-D reactions from Wurzel2022.
    Source: "Atzeni, Meyer-ter-Vehn - The Physics of Inertial Fusion", chapter 1, Fig 1.13.
    E_COM in [keV], cross sections sigma in [barn].
    """
    file_path = get_fusion_cross_section_data_file_path(reaction=reaction)
    data = np.loadtxt(file_path, delimiter=',')
    E_COM_list = data[:, 0]
    sigma_list = data[:, 1]
    # log_interp_fun = interp1d(np.log10(E_COM_list), np.log10(sigma_list), kind='cubic', bounds_error=False)
    log_interp_fun = interp1d(np.log10(E_COM_list), np.log10(sigma_list), bounds_error=False)
    sigma_v_interped = 10.0 ** (log_interp_fun(np.log10(E_COM)))
...
def get_sigma_v_fusion_approx(T, reaction='D_T_to_n_alpha', n=None):
    ...
    if reaction == 'D_D_to_p_T_n_He3':
        reactions = ['D_D_to_p_T', 'D_D_to_n_He3']
        return sum([get_sigma_v_fusion_approx(T, reaction=reaction, n=n) for reaction in reactions])
...
def get_sigma_v_fusion_approx_numeric_integration(T_keV_array, reaction='D_T_to_n_alpha'):
    """
    Approximation of <sigma*v> (Maxwell averaged reactivity),
    numerically integrating sigma(E) in its approximated form.
...
def get_sigma_v_fusion_fit(T, reaction='D_T_to_n_alpha', use_resonance=True):
    """
    ...
    if reaction == 'D_T_to_n_alpha':
        ...
        reactions = ['D_D_to_p_T', 'D_D_to_n_He3']
        return sum([get_sigma_v_fusion_fit(T, reaction=reaction) for reaction in reactions])
    else:
        raise ValueError('invalid reaction: ' + reaction)
...
def get_fusion_reactivity_data_file_path(reaction='D_T_to_n_alpha'):
...
def get_sigma_v_fusion_sampled(T, reaction='D_T_to_n_alpha'):
    ...
    Most from Atzeni2004 Book (Chapter 1, Fig. 1.5), except the separate D-D reactions from Wurzel2022.
    T in [keV], reactivity sigma*v in [m3/s].
    """

    if reaction == 'p_B_to_3alpha':  # change default data set for pB
        reaction += '_Sikora2016newdata'
        file_path = get_fusion_reactivity_data_file_path(reaction=reaction)
...
def get_fusion_sigma_v_E_reaction(T, reaction='D_T_to_n_alpha', energy_type='all'):
    ...
    if reaction == 'D_D_to_p_T_n_He3':
        reactions = ['D_D_to_p_T', 'D_D_to_n_He3']
        return sum([get_sigma_v_fusion_fit(T, reaction=reaction) * E_fun(reaction=reaction) for reaction in reactions])

    else:
        sigma_v_E = get_sigma_v_fusion_fit(T, reaction=reaction) * E_fun(reaction=reaction)
...
def get_lawson_parameters(ni, Ti, settings, reaction='D_T_to_n_alpha'):
    ...
    T in [keV], ni in [m^-3].
    """
    sigma_v_fusion = get_sigma_v_fusion_fit(Ti, reaction=reaction)
    MeV_to_J = define_electron_charge() * 1e6
